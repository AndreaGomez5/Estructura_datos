...
class DetectiveDigital:
    def __init__(self):                                   
        ...
        self.modelo_ml = self.entrenar_modelo()           # Entrena el clasificador

    def crear_red(self):
        nombres = [NodoServidor(n) for n in "ABCDE"]            # Crear nodos
        for a, b in zip(nombres, nombres[1:]): a.siguiente = b  # Enlazarlos uno a uno
        return nombres[0]                                       # Retornar el nodo inicial
    ...
    def entrenar_modelo(self):
        X, y = [], []          # Inicializa entradas y salidas
        for _ in range(1000):  # Simula 1000 ejemplos de eventos
            h, n, c = random.randint(0,23), random.randint(0,4), random.randint(0,100)  # Genera hora, servidor, tráfico
            X.append([n,h,c])   # Guarda el vector de entrada 
            y.append(1 if c > 70 or h >= 22 or h <= 3 else 0)   # Define si es hackeado o no
        modelo = DecisionTreeClassifier()   # Crea el modelo ML
        modelo.fit(X,y)  # Lo entrena con los datos simulados
        return modelo    # Retorna el modelo para usarlo luego
